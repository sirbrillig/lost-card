...
export class Preloader extends Scene {
	...
	init() {
		...
		this.add.rectangle(0, 0, 200, 32).setStrokeStyle(1, 0xffffff);
		...
		this.load.on("progress", (progress: number) => {
			//  Update the progress bar (our bar is 200 wide, so 100% = 200)
			bar.width = 4 + 200 * progress;
		});
	}
	...
	preload() {
		...
		this.load.spritesheet("player-hit", "player-hit.png", {
			frameWidth: 32,
			frameHeight: 32,
		});
		this.load.spritesheet("orange_boom", "orange_boom.png", {
			frameWidth: 32,
			frameHeight: 32,
		});
		...
		this.load.spritesheet("plant-power", "Vine3.png", {
			frameWidth: 8,
			frameHeight: 10,
		});
